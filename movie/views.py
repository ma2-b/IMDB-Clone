...
def index(request):
	...
	if query:
		url = 'http://www.omdbapi.com/?apikey=2005286b&s=' + query
...
def pagination(request, query, page_number):
	url = 'http://www.omdbapi.com/?apikey=2005286b&s=' + query + '&page=' + str(page_number)
	response = requests.get(url)
	movie_data = response.json()
...
def movieDetails(request, imdb_id):
	...
	if Movie.objects.filter(imdbID=imdb_id).exists():
		...
		url = 'http://www.omdbapi.com/?apikey=2005286b&i=' + imdb_id
		response = requests.get(url)
		movie_data = response.json()
